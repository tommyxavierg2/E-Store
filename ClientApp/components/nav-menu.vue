<template>
    <div class="main-nav">
        <nav class="navbar navbar-expand-md navbar-dark bg-dark">
            ...
            <router-link class="navbar-brand" to="/"><icon :icon="['fab', 'microsoft']"/> EStore</router-link>
                ...
                <div :class="'collapse navbar-collapse' + (!collapsed ? ' show':'')" v-show="!collapsed">
                    <ul class="navbar-nav mr-auto">
                        <li class="nav-item" v-for="(route, index) in routes" :key="index">
                            <router-link :to="route.path" exact-active-class="active"> {{route.name}} </router-link>
                        </li>
                    </ul>
                </div>
            </transition>
